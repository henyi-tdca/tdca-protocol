/-
  ...
  ============================================================================
  状态：**骨架 + 证明尝试——本机已机验（2026-09-22，Lean 4.34.0-rc2）**：全件 exit 0、零 sorry；非 CI 机验
        → 机验实测**无 `sorry`**；对外（CI 复跑前）不得称「已证明」（表述与机验纪律 / 22）。
...
⚠️ **范围限定（如实）**：本定理只覆盖「`ECS` → `M_A`」这一段传递；
   ...
   **实测确认非引理可见性问题**（`Monotone` / `mul_le_mul_of_nonneg_left` / `add_le_add_left` 三者**均成功 elaboration**）。
   现改用 **`add_le_add (le_refl M_A_base) hstep`**（**不依赖 `left`/`right` 命名**，直接匹配目标形态）。
    ⚠️ **修正后已机验（2026-09-22 本机）：exit 0、零 sorry**（V6 复跑即本机复跑；CI 复跑仍待）。 -/
...
/-
  ...
  **证明义务清单（sorry 清零 checklist）——M-8 Lean 化后状态**：
   [M8-a] 本文件**已给完整证明尝试**（5 条定理）；**2026-09-22 本机机验 exit 0、零 sorry** → 义务**本机层面已勾销**（CI 复跑仍待）。
          已知实现层风险点：
   ...
   ⚠️ **本文件 `sorry` 计数 = 0（2026-09-22 本机机验确认，编译器零 sorry 告警）**；CI 复跑前不得称「已证明」（表述与机验纪律 / 22）。
