...
# 工作区路径：三段式自动探测（① TDCA_WS 环境变量覆盖 → ② 仓内相对探测 → ③ 均不成立明确报错退出 2，不静默空跑）
def _resolve_cop_dir() -> Path:
    ws = os.environ.get("TDCA_WS")
    if ws:
        d = Path(ws).expanduser() / "tdca-thinktank" / "research" / "topics" / "thinking-protocol" / "cop-library"
        if d.is_dir():
            return d
        print("[FAIL] TDCA_WS 已设置但其下 cop-library 不存在: %s" % d, file=sys.stderr)
        print("       不静默空跑：请核 TDCA_WS 指向（应含 tdca-thinktank/.../cop-library）", file=sys.stderr)
        sys.exit(2)
    # ② 仓内相对探测：本文件位于 <仓>/tools/connector-v2/ → 仓根 = parents[2]
    d = Path(__file__).resolve().parents[2] / "docs" / "cop-library"
    if d.is_dir():
        return d
    print("[FAIL] 未找到 cop-library：TDCA_WS 未设置，仓内相对探测亦未命中: %s" % d, file=sys.stderr)
    print("       不静默空跑：设置 TDCA_WS 指向含 tdca-thinktank/.../cop-library 的工作区", file=sys.stderr)
    sys.exit(2)


COP_DIR = _resolve_cop_dir()
